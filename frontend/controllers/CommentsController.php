...
class CommentsController extends AppController
{
    ...
    function actionNewComment()
    {
        ...
        if ($newComment !== false) {
            ...
            $isReplyComment = $newComment['reply_comment_id'] == null ? false
                : true;
            $idAuthor
                = $class::AuthorId(Yii::$app->request->post('page_post_id'));
            $this->sendNotificationsNewComments(Yii::$app->request->post('page_post_id'),
                $newComment, $type);
    ...
    {
        $comment = new Comments();
        $comments = $comment->getComments(Yii::$app->request->post('id'),
            Yii::$app->request->post('type'), Yii::$app->request->post('page'));
    ...
    {
        $AuthorCommentId
            = Comments::AuthorCommentId(Yii::$app->request->post('id'));
        if (Yii::$app->request->isAjax
            && Yii::$app->userAccess->access('edit_frontent') == 1
            || $AuthorCommentId == Yii::$app->user->id
        ) {
            $comment = new Comments();
    ...
    {
        $AuthorCommentId
            = Comments::AuthorCommentId(Yii::$app->request->post('comment_id'));
        if (Yii::$app->request->isAjax
            && $AuthorCommentId == Yii::$app->user->id
        ) {
            $comment = new Comments();
    ...
    function actionBlockedComment()
    {
        if (Yii::$app->request->isAjax
            ...
            $res
                = $userBlockedComment->userBlockedComment(Yii::$app->request->post('id'));
    ...
    function sendNotificationsNewComments($postId, $comment, $type)
    {
        ...
        foreach ($subscribes as $subscriber) {
            $content = $emailNotifications->PrepareEmail($subscriber['email'],
                'New comments at lykke.com', [
                    '@[name]'             => $subscriber['first_name'],
                    '@[post_title]'       => $subscriber['post_title'],
                    '@[comment_author]'   => $comment['first_name'].' '
                        .$comment['last_name'],
                    '@[date]'             => date("M d, g:i a",
                        strtotime($comment['date'])),
                    '@[comment_text]'     => \yii\helpers\StringHelper::truncate($comment['comment'],
                        358, '...'),
                    '@[post_link]'        => Yii::$app->urlManager->hostInfo.'/'
                        .Yii::$app->params['uri_'.$type].'/'
                        .$subscriber['post_url'],
                    '@[unsubscribe_link]' => Yii::$app->urlManager->hostInfo.'/'
                        .Yii::$app->params['uri_'.$type].'/'
                        .$subscriber['post_url'].'#unsubscribe',
                    '@[year]'             => date('Y'),
                ]);
