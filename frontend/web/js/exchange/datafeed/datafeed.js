...
Datafeeds.UDFCompatibleDatafeed = function(storage, updateFrequency) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getMarks = function(symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
    if (this._configuration.supports_marks) {
        this._storage.getMarks(symbolInfo.ticker.toUpperCase(), rangeStart, rangeEnd, resolution)
            .done(function(response) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getTimescaleMarks = function(symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
    if (this._configuration.supports_timescale_marks) {
        this._storage.getTimescaleMarks(symbolInfo.ticker.toUpperCase(), rangeStart, rangeEnd, resolution)
            .done(function(response) {
                onDataCallback(JSON.parse(response));
            })
            .fail(function() {
...
Datafeeds.UDFCompatibleDatafeed.prototype.searchSymbols = function(searchString, exchange, type, onResultReadyCallback) {
    ...
    if (this._configuration.supports_search) {
        this._storage.search(MAX_SEARCH_RESULTS, searchString.toUpperCase(), type, exchange)
            .done(function(response) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    ...
    if (!this._configuration.supports_group_request) {
        this._storage.resolveSymbol(symbolName ? symbolName.toUpperCase() : '')
            .done(function(response) {
                var data = JSON.parse(response);
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
    ...
    this._storage.getHistory(symbolInfo.ticker.toUpperCase(), rangeStartDate, rangeEndDate, resolution)
        .done(function(response) {
            var data = JSON.parse(response);
...
Datafeeds.UDFCompatibleDatafeed.prototype.getQuotes = function(symbols, onDataCallback, onErrorCallback) {
    this._storage.getQuotes(symbols)
        .done(function(response) {
            var data = JSON.parse(response);
            if (data.s === 'ok') {
...
Datafeeds.SymbolsStorage.prototype._requestFullSymbolsList = function() {
    ...
    for (var i = 0; i < this._exchangesList.length; ++i) {
        ...
        this._datafeed._storage.getFullSymbolsList(exchange)
