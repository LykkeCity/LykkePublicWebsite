    ...
    <div class="user_badge__message">
      <div class="message_card__inner">
        <div class="user_badge__title"><a href=""><?=$comment['first_name']?> <?=$comment['last_name']?></a>
          ...
          <?=$comment['lykke_user_id'] == $idAuthor ? '<span class="label label--primary">author</span> <span class="middot">&middot;</span>' : ''?>

          <span class="message_card__date"><?=date("g:i A F d, Y", strtotime($comment['date']));?></span>

          <?=$comment['edited'] == 1 ? '<span class="middot">&middot;</span>   <div class="label label--dark label--text label-edited">edited <i class="icon icon--edit_alt"></i></div>' : ''?>

          <?=$comment['deleted'] == 1 ? '<span class="middot">&middot;</span>  <span class="label label--dark label--text">deleted</span>' : ''?>

          <?=$comment['spam'] > 0 ? '<span class="middot ">&middot;</span>   <div class="label label--text label--red label-spam">Suspicious comment</div>' : ''?>
          ...
          <p class="comment-text"><?=$comment['comment']?></p>
                ...
                <div class="card__actions">
                  <?php if (!Yii::$app->user->isGuest) { ?>
                      ...
                      <?php if ($comment['lykke_user_id'] == Yii::$app->user->id && strtotime(date('Y-m-d H:i:s')) <= strtotime($comment['date'].'+1 day') && $comment['deleted'] == 0){ ?>
                        &middot;
                        <a href="" data-id="<?=$comment['id']?>" class="action_link action_link-edit">Edit</a>
                      <?php } ?>

                      <?php if ((Yii::$app->userAccess->access('edit_frontent') == 1 || $comment['lykke_user_id'] == Yii::$app->user->id) && $comment['deleted'] == 0){ ?>
                         &middot;
                         <a href="" data-id="<?=$comment['id']?>" class="action_link action_link-delete">Delete</a>
                       <?php } ?>

                        &middot;
                        <a href="" data-id="<?=$comment['id']?>" class="action_link action_link-spam">Spam</a>


                      <?php if (Yii::$app->userAccess->access('edit_frontent') == 1){ ?>
                        &middot;
                        <a href="" data-id="<?=$comment['lykke_user_id']?>" class="action_link action_link-block-user">Block user</a>
                      <?php } ?>
                  <? }else{ ?>
                  ...
                  <? if (!empty($comment['reply_comment'])) {?>
                    <span class="action_text  pull-right"><?=count($comment['reply_comment'])?> response</span>
                  <? } ?>
