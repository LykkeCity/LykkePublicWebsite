...
class CommentsSubscribe extends ActiveRecord {
  ...
  function subscribe($pagePostId, $type){
    $subscribe = self::findOne(['lykke_user_id' => $pagePostId, 'lykke_user_id' => Yii::$app->user->id, 'type' => $type]);
    if (empty($subscribe)) {
  ...
  function getSubscribers($pagePostId, $type){
    ...
    return  (new Query)->select("lu.first_name,
      ...
      ->leftJoin($class::tableName().' bp', 'bp.id = '.$pagePostId)
      ->where("sub.page_post_id = ".$pagePostId." AND sub.subscribe = 1 AND sub.lykke_user_id != ".Yii::$app->user->id)->createCommand()->queryAll();
